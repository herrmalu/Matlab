% function [data, fileName] = csvimport(fileName)

%function data = csvimport( varargin )
...
%
% Function has not been tested on badly formatted CSV files.
...
%   [fileName, filePath] = dir('*.csv'); %  [fileName, filePath] = uigetfile( '*.csv', 'Select CSV file' );
...
%   end
  ...
  for ii =  1 : numel( params )
    result        = STRNCMP( params{ii}, lcValidParams );
    ...
    %an error
    if numel( result ) > 1
      exresult    = STRNCMP( params{ii}, validParams, 'exact' );
      if ~isempty( exresult )
        result    = exresult;
      else
...
if colMode
  if ~p.noHeader
    ...
    for ii = 1 : nHeaders
      result      = STRNCMP( p.columns{ii}, rowData );
      if isempty( result )
        fclose( fid );
      ...
      elseif numel( result ) > 1
        exresult  = STRNCMP( p.columns{ii}, rowData, 'exact' );
        if numel( exresult ) == 1
          result  = exresult;
        else
